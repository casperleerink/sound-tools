...
An effect is any tool with an `audio` input and an `audio` output. Today that is the built-in `filter`, `compressor` and `eq`, whose records are in `agent-docs/filter.md`, `agent-docs/compressor.md` and `agent-docs/eq.md`, and the `plugin` tool, which is the same record as an instrument; `agent-docs/plugins.md` says where the ids come from. The file name is yours: lowercase letters, digits, `-` and `_`, and not `instrument`.
