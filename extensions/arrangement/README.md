...
How the rack grows. Effects come after the instrument: more ids from `device_slots`, in rack order, and nothing else changes, because a card is already made per slot. The mixer controls of the track (gain, pan, mute) are not devices. They get a fixed section at the right end of the row in `TrackPanel::render`, after the rack, which then scrolls on its own. None of this is built, and there are no empty places for it: no effects, sends, instrument picker or reordering.
