...
| `view.rs` | The card of a plugin in a rack, 200 pt: `Open window` at the top of its body and `CLAP · <maker>` on the value line of its second row. No expand, because nothing is hidden. An effect gets power and close from the rack, in its frame. And what a rack calls one. |
