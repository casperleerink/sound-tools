...
The EQ owns no children, so an instance is one file: `<name>.json` in a track folder, named in the track's `effects`. `bands` is a list of at most four bands, band 1 first. A band or a field left out is reset to the default of that band, not kept, so an agent writes the whole list. The runtime writes every field of every band when it saves. An unknown field, a value out of range, an unknown shape or a fifth band does not load. The instance then keeps its last valid state, and the problem names the file, the band and the field.
...
- Q above 0.71 gives a cut or a shelf a bump at its frequency, and a shelf a dip on its other side, as in the analog shapes. A cut keeps the whole range, because a resonant cut is a normal tool: it peaks at its Q, +25 dB at Q 18. A shelf uses Q up to 1.5 (`SHELF_MAX_Q`), where its bump is at most 3.2 dB past its gain; uncapped, a shelf of +15 dB would peak at +38 dB at Q 18. The saved `q` is kept as written, only the shelf math uses less, so a shelf turned into a bell gets its Q back. `response` and the curve use the same cap.
...
Measured September 26, 2026 on an Apple Silicon laptop, dev profile with `opt-level = 3`, 48 kHz, offline, while other builds kept the load average near 29: 100 EQs with four bands at work, each fed by its own noise source, render 2.8 times faster than realtime, and 2.2 times with every band gliding all the time. The Filter's own measurement gave 2.4 times under the same load, against 7.7 on a quiet machine. The measured response is within 0.0002 dB of `response` for every shape, gain and Q of the test from 50 Hz to 16 kHz, and several bands together within 0.0001 dB.
