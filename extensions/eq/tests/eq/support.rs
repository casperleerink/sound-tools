//! A rig of one source into one EQ into the device, and what the tests measure with.

use std::f64::consts::TAU;

use eq::{Band, Eq, EqState, Shape};
use sound_core::{
...
/// A source, an EQ and the device, rendering offline.
pub struct Rig {
...
impl Rig {
    pub fn new(state: EqState, signal: Signal) -> Self {
        Self::at_rate(state, signal, SAMPLE_RATE)
    }

    pub fn at_rate(state: EqState, signal: Signal, sample_rate: u32) -> Self {
        ...
        let eq = edit.add_processor("eq", Eq::new(state)).unwrap();
    ...
    pub fn update(&mut self, state: EqState) {
        self.control.update(self.eq, state).unwrap();
    }
}
...
/// The gain of an EQ with this record at `hz`, measured: a quiet sine through it, once the
/// EQ has settled, in dB.
pub fn measured_db(state: EqState, hz: f64) -> f64 {
    measured_db_at(state, hz, SAMPLE_RATE)
}

/// The same at another sample rate.
pub fn measured_db_at(state: EqState, hz: f64, sample_rate: u32) -> f64 {
...
/// A band that is on.
pub fn band(shape: Shape, frequency_hz: f32, gain_db: f32, q: f32) -> Band {
    Band {
        on: true,
        shape,
        frequency_hz,
...
/// An EQ with these bands on, from band 1, and every other band off.
