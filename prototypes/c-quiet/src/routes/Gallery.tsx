...
import { wireOpacity, wirePath } from "@/core/Canvas";
...
function ToolCards() {
  ...
  const kinds: PortKind[] = ["audio", "event", "mod"];
  const kindName: Record<PortKind, string> = { audio: "Audio", event: "Events", mod: "Modulation" };
  return (
    <Section title="Tool card and ports" hint="Header: accent dot, instance name, view tabs, close on hover. Ports: disc = audio, ring = events, diamond = modulation. Labels on hover.">
      <div className="flex flex-wrap items-start gap-12 rounded-[10px] border border-alpha/10 bg-gray-100 p-8 pl-12">
        <ToolFrame
          instanceName="Drone"
          typeName="Additive"
          accent="blue"
        ...
        <ToolFrame instanceName="Output" typeName="Mixer" accent="teal" views={[{ id: "strips", label: "Strips" }]} activeView="strips" onViewChange={() => {}} inputs={[{ id: "in-1", label: "In 1", kind: "audio" }]} width={240} selected>
          <div className="text-gray-800 text-xs">Selected card: lighter border.</div>
        </ToolFrame>
        <div className="flex flex-col gap-2 rounded-lg border border-alpha/10 bg-gray-200 p-4">
          <span className="text-gray-700 text-xs">Wires: one weight, the source instance's colour, the kind in the opacity</span>
          <svg width={220} height={110} aria-hidden>
            {kinds.map((kind, i) => (
              <path key={kind} d={wirePath({ x: 8, y: 16 + i * 36 }, { x: 212, y: 24 + i * 36 })} fill="none" stroke={accents.peach} strokeOpacity={wireOpacity[kind]} strokeWidth={1.5} strokeLinecap="round" />
            ))}
          </svg>
          <div className="flex flex-col gap-0.5 text-2xs text-gray-800">
            {kinds.map((kind) => (
              <span key={kind}>
                {kindName[kind]} · {Math.round(wireOpacity[kind] * 100)}%
              </span>
            ))}
          </div>
        </div>
