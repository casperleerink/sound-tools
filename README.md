...
The folder is the project. When it is empty or missing, the app makes the default project in it: 120 bpm, 4/4, one track with a synth. There is no save: every finished edit is written to the folder.
...
- `AGENTS.md` (and `CLAUDE.md`, which imports it): the app writes it into the project. It explains the folder layout, the record formats and the bar math of this project. Agents read it by themselves.
...
- `runtime <folder> --inspect`: prints the tempo, every track and every clip with its bar range. It works next to the running app.
...
- `--render` writes a WAV offline: 48 kHz, stereo, 32-bit float.
