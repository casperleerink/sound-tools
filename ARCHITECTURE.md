...
- The host needs the application to open a window and to take one down, and the moments that find a plugin nobody names any more do not have it: `Plugins::poll` and the drop of the host free the plugin's view and leave the window to `Plugins::settle_windows`, which whoever polls calls with the application in hand. Nothing of GPUI runs while the table of plugins is borrowed, because a card that is drawn asks the host what its plugin has.
