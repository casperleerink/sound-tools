...
- Choosing replaces the whole record of the slot as one undo step, named after what was chosen (`Choose Six Sines`). Undo brings back what was there, and for a plugin as it sounded: 4a saves a plugin on its way out. A plugin that is chosen gets a `state_asset` no other record of the project uses, from the plugin's name (`six-sines`, `six-sines-2`). Whoever writes a record by hand still chooses the name themselves and two records may still share one. Only records are looked at, so a state file left by a record that is gone is taken up again by the next plugin that wants that name, which gives a patch back rather than losing one.
...
- The host needs the application to open a window and to take one down, and the moments that find a plugin nobody names any more do not have it: `Plugins::poll` and the drop of the host free the plugin's view and leave the window to `Plugins::settle_windows`, which whoever polls calls with the application in hand. That call also gives a window the size its plugin asked for, and frees the view of a window that went without saying so, because GPUI's close callback is best effort. Nothing of GPUI runs while the table of plugins is borrowed, because a card that is drawn asks the host what its plugin has; what a card asks is a number the host already had, never a call into the plugin, and it gives up rather than wait for a table a plugin's own call has borrowed.
- The application ends with the main window and not with the last one. A plugin's window is a window of this application too, so `cx.on_window_closed` would have kept the process alive behind an empty plugin window when the composer closed the project.
