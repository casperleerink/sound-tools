...
#### Hosting plugins, decided September 20 and 21, 2026 with steps 4a, 4b and 5a
...
Done with step 5a: a track whose record says `"format": "vst3"` plays its clips and the live keyboard through that plugin, in the running project and in an offline render; the composer picks it in the same picker; its state survives close and reopen and an offline render before equals one after, byte for byte, whether its samples are loaded or not; a missing VST 3 plugin is the same reported problem as a missing CLAP one; and no plugin of this machine is ever looked at on the thread that draws. What 5b owes: a VST 3 plugin's own window. `Vst3Plugin::gui` answers `None` and a card says the plugin has no window of its own, so 5b writes one `backend::PluginGui` over `IPlugView` with an `IPlugFrame` for the sizes, and touches nothing else of the host. Without a window nothing in 5a can change a real plugin's own settings by hand, so what 5a shows of state is that it is saved, read back and identical; what a composer moves in a plugin's interface is 5b's to show.

Not built: effects, AU, a plugin sandbox, latency compensation, parameter automation, a generic parameter view, presets and program lists, MIDI out of a plugin, more than the first event input and the first stereo output, the transport a plugin can read, and finding a plugin installed while the app runs. Two known holes in the scan cache, left for a later step: the key reads the first file in a bundle's binary folder instead of the executable the `Info.plist` names, and it does not include the architecture of the host; and two runtimes writing the cache at once can garble it, which costs a rescan and nothing else.
