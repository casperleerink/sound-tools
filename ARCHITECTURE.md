...
| The realtime checks pass on every `process` function | Pass | Three processors outside tests: `Tone`, `Synth`, `Sequencer`. Each runs under `RTSAN_ENABLE=1` in the tests of its crate, 221 tests. The check added the real-project tests of the runtime to that run. |
