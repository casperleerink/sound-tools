...
- **The steadiness is in the transport**, after the tempo, and only when the project has a fit. The pill is full, so this is the argument: it is the same kind of thing as the tempo, a number about the time of the whole project that drags the same way, and a project that was never fitted has exactly the transport it always had. A drag is one gesture and one undo step, heard and seen while it runs; the arrows step by five percent and with shift by one.
