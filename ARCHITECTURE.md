...
- The track panel: nobody has turned a knob by ear yet, only by event injection and offline renders. The rack does not scroll, which one card does not need. A knob has no fine drag with a modifier and no typed value. Track selection has no keys of its own while a clip is selected: click a header, or deselect the clip.
