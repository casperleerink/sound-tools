...
- The frames are simulated exactly as `Clock` computes them, at a fixed 48 kHz. A tempo map is saved with no sample rate, so building it against the device of the moment would make one take two different files on two machines, and the brief asks for the same bytes on every run. At another sample rate the clock rounds each tempo change down to a whole frame, so a map with many steps drifts slowly against the take; fitting again on that machine takes it away. This is a known gap.
...
- **The fit is in the project menu**, `Fit tempo to take`, next to `Add track`, and at 40 % when the selected clip was not recorded. It belongs there because a fit is about the whole project: it rewrites the tempo map every other part follows. The arrangement publishes the selected clip to the session, next to the selected track, so the window can offer it without the arrangement knowing anything about takes.
...
Not built: tempo ramps, time signature changes inside a piece, steadiness per section, more than one fit per project, a fit of a take whose clip is not the first to name it, overdub and merging takes, audio, a fit that runs off the thread that draws, and a fitted map that is exact at a sample rate other than 48 kHz.
