...
- A VST 3 state asset holds both states VST 3 keeps, the component's and the controller's, as a preset file does.
...
An offline render of a sampled instrument is repeatable, but not from the first block after the samples are cold. Measured: a render made right after the sample files left the file cache is silent, and every render after that is byte for byte the same as the one before it, and the same across a close and a reopen that wrote the state assets. These instruments stream from disk and an offline render does not wait for one: it runs faster than realtime and takes what the plugin has. That is the plugin and not the host, and nothing this host can do changes it; what holds is that two renders of a project whose samples are loaded are identical, which is what a test of a render compares.

Done with step 5a: a track whose record says `"format": "vst3"` plays its clips and the live keyboard through that plugin, in the running project and in an offline render; the composer picks it in the same picker; its state survives close and reopen and an offline render before equals one after, byte for byte, once its samples are loaded; a missing VST 3 plugin is the same reported problem as a missing CLAP one; and no plugin of this machine is ever looked at on the thread that draws. What 5b owes: a VST 3 plugin's own window. `Vst3Plugin::gui` answers `None` and a card says the plugin has no window of its own, so 5b writes one `backend::PluginGui` over `IPlugView` with an `IPlugFrame` for the sizes, and touches nothing else of the host. Without a window nothing in 5a can change a real plugin's own settings by hand, so what 5a shows of state is that it is saved, read back and identical; what a composer moves in a plugin's interface is 5b's to show.

Not built: effects, AU, a plugin sandbox, latency compensation, parameter automation, a generic parameter view, presets and program lists, MIDI out of a plugin, more than the first event input and the first stereo output, the transport a plugin can read, and finding a plugin installed while the app runs.
