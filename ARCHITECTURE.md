...
- Views hold the session and typed instances, read state when they render and keep no copy of saved state. Every edit goes through `Session::edit`, which puts an error into one notice that the window shows as a quiet line. Files that are not live show as a second line that names `problems.txt`.
...
- Done September 19, 2026: the application window with the session bridge, the view registry, the read-only arrangement view, the transport and the project menu. Not built: editing clips and notes in the window, the note editor, following the playhead when it leaves the view, device switching, a tempo display.
