...
- A slot with no plugin copies its input to its output. That one line is what makes "a missing effect is reported and the track plays on through the rest of the chain" true, with nothing in the arrangement knowing why. For an instrument the input is connected to nothing, so it is the silence it always was.
...
- Not built: reordering in the window, bypass, a wet and dry amount, built-in effects of our own, sends and buses, effects on a master or on the device, sidechain inputs, more than the first stereo input and output, latency compensation, and MIDI effects.
