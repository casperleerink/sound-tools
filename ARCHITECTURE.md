...
| Effects | Two or three of our own, such as delay, filter and reverb. Not built: a track holds third-party effect plugins since step 6 of the second milestone, through the plugin host. |
...
- Not built: a plugin window that follows a drag of its edge, remembering where a window sat or whether it was open, a floating window for a plugin that only floats, keeping a plugin's window above the main one (CLAP's `set_transient`, which needs a handle to our view that outlives the plugin's window and on macOS the main window goes first), and a search box in the picker.
...
- The cache belongs to the machine and not to a project: `~/Library/Caches/sound-tools/plugins.json`. A project folder in git must not carry a list of what one laptop happens to have. A bundle is remembered by the modified time and size of the binary inside it, so a plugin that was installed or updated is looked at again and nothing else is. A bundle that crashed or hung is remembered as such and is not tried again on every start; `runtime --plugins` looks at everything again and writes the result, which is how one that was fixed comes back.
...
- Resizing by dragging stays unbuilt for both formats. VST 3 says how (`canResize`, `checkSizeConstraint`, `onSize`) and so does CLAP, but it needs a GPUI resize wired back into the plugin on every frame of a drag, which is more than the line step 4b drew.
...
What `restartComponent` asks for, measured over the five VST 3 instruments of this machine on September 21, 2026. None of them asks for `kReloadComponent` or `kIoChanged`, which are the two that would need a deactivate and an activate, so this build still only tells the composer that a plugin asked to be started again and does not do it. LABS, Numa Player and Crow Hill Origins each send `kParamValuesChanged` once, when their saved state is read back, and nothing else, not even while a composer works in their window. Splice INSTRUMENT sends `kParamTitlesChanged`, `kParamIDMappingChanged` with `kParamValuesChanged`, and `restartComponent(0)`, which asks for nothing at all. Vital cannot be loaded on this machine. Two things follow, both left for later: this build reports "asked to be started again" for `kParamTitlesChanged` and `kParamIDMappingChanged`, which need no restart and which a host with no parameter view has nothing to do about; and answering `kParamValuesChanged` properly, by reading every parameter of the controller back and giving it to the processor, is now one call away from what the edit path already does.
...
- Not built: reordering in the window, bypass, a wet and dry amount, built-in effects of our own, sends and buses, effects on a master or on the device, sidechain inputs, more than the first stereo input and output, latency compensation, and MIDI effects. A `project.json` connection written by hand into the `audio` input of a record in the instrument slot is passed through to that slot's output while its plugin is missing, because the slot cannot know what its input is for.

Not built: AU, a plugin sandbox, latency compensation, parameter automation, a generic parameter view, presets and program lists, MIDI out of a plugin, more than the first event input, the first stereo input and the first stereo output, the transport a plugin can read, a plugin window that follows a drag of its edge, key events passed to a view (a plugin's own `NSView` is in the responder chain of our window, so typing in it works through AppKit), remembering where a window sat or whether it was open, keeping a plugin's window above the main one, and finding a plugin installed while the app runs. `Vst3Gui` is `Send` and `Sync` through the COM types the `vst3` crate generates although it belongs to the thread the user interface lives on; nothing sends one, and making that impossible is a later tidy. Two known holes in the scan cache, left for a later step: the key reads the first file in a bundle's binary folder instead of the executable the `Info.plist` names, and it does not include the architecture of the host; and two runtimes writing the cache at once can garble it, which costs a rescan and nothing else. One plugin of this machine, Numa Player, draws, takes the mouse and keeps its state but its output is about -175 dBFS in this host whatever it is given; it asks the host for nothing at all while a composer loads a sound into it, so nothing of the window design is in its way. That is an audio-path question from step 5a, and it is written up in the pull request of 5b.
...
- One window per open plugin, beside the main one, since step 4b of the second milestone. Its root view draws nothing; the plugin's own view fills it. The plugin host owns them, see "Hosting plugins".
