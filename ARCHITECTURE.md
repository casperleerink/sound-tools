...
- The view comes from the plugin's edit controller, `createView(ViewType::kEditor)`, which is also the only way the format has of asking whether a plugin has a window at all. So `is_offered` makes a view, asks it `isPlatformTypeSupported(kPlatformTypeNSView)` and lets it go again, once, while the plugin loads. That is what keeps a card honest about a plugin with no window, exactly as a CLAP card is, and it costs a plugin's whole interface being built once. Measured on this machine with `--inspect` on a project of one plugin, without the probe and with it: LABS 0.24 s to 0.31 s, Numa Player 0.02 s to 0.13 s, Splice INSTRUMENT 0.25 s to 0.38 s, and Crow Hill Origins 0.18 s to 1.16 s, because Origins builds a large textured interface. So a project pays up to a second per VST 3 plugin at the moment it opens. The alternative, offering the window whenever the plugin has an edit controller and reporting a problem if `createView` then gives nothing, costs nothing and greys no button; it was not taken, because a card that offers a window a plugin does not have is a difference a composer can see, which this step was told not to make. If that second ever matters, the probe can move from the load to the first poll, which puts it after the project is on screen.
...
Not built: effects, AU, a plugin sandbox, latency compensation, parameter automation, a generic parameter view, presets and program lists, MIDI out of a plugin, more than the first event input and the first stereo output, the transport a plugin can read, a plugin window that follows a drag of its edge, key events passed to a view (a plugin's own `NSView` is in the responder chain of our window, so typing in it works through AppKit), remembering where a window sat or whether it was open, keeping a plugin's window above the main one, and finding a plugin installed while the app runs. Two known holes in the scan cache, left for a later step: the key reads the first file in a bundle's binary folder instead of the executable the `Info.plist` names, and it does not include the architecture of the host; and two runtimes writing the cache at once can garble it, which costs a rescan and nothing else. One plugin of this machine, Numa Player, draws, takes the mouse and keeps its state but its output is about -175 dBFS in this host whatever it is given; that is an audio-path question from step 5a, not a window one, and it is written up in the pull request of 5b.
