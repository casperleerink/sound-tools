...
- `TempoMap`: the time signature and a list of tempo changes. Steps only, no ramps. The first change is at tick 0 and the ticks go up. This is the saved form. `with_tempo_at(tick, bpm)` gives the same map with the tempo change that starts there set, or `None` when there is none: it cannot fail, because only a tempo changes and the ticks keep their order. `change_at(tick)` is the change in effect at a tick. `with_change_at(tick)` adds a change there with the tempo that already plays, so nothing sounds different, and `without_change_at(tick)` removes one; both give `None` when there is nothing to do, and the change at tick 0 stays. `Tempo` displays short: `120`, `93.5`, `120.125`.
