...
/// The doc of `project.json`. The core brings it, because `project.json` is core and not an
/// extension. [`Registry::new`] puts it in the list, so it goes through the same name check as
/// every other doc and an extension cannot take its name.
pub(crate) const PROJECT_FILE_DOC: AgentDoc = AgentDoc {
    name: "project-json",
