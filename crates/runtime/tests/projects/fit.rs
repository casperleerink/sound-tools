...
fn another_downbeat_and_another_time_signature_rebuild_the_grid() {
    ...
    let file = r#"{"format": 1, "extensions": ["arrangement", "compressor", "eq", "filter", "fit-tempo", "instrument", "plugin-host", "reverb", "tone"], "tempo_map": {"time_signature": "3/4", "tempo_changes": [{"tick": 0, "bpm": 120.0}]}, "connections": []}"#;
...
fn write_real_project(folder: &std::path::Path, plugin_id: &str) {
    ...
    write(
        "project.json",
        r#"{"format": 1, "extensions": ["arrangement", "compressor", "eq", "filter", "fit-tempo", "instrument", "plugin-host", "reverb", "tone"], "tempo_map": {"time_signature": "4/4", "tempo_changes": [{"tick": 0, "bpm": 120.0}]}, "connections": []}"#.to_string(),
    );
    write(
