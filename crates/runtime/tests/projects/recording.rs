...
fn a_take_becomes_a_clip_that_renders_what_was_heard() {
    ...
    recorder.harness.write_and_apply(
        "project.json",
        r#"{"format": 1, "extensions": ["arrangement", "compressor", "eq", "filter", "fit-tempo", "instrument", "plugin-host", "tone"], "tempo_map": {"time_signature": "4/4", "tempo_changes": [{"tick": 0, "bpm": 120.0}, {"tick": 3840, "bpm": 150.0}]}, "connections": []}"#,
    );
