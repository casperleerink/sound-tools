...
/// How much of what the track plays is high, from its left channel: the mean step from sample
/// to sample over the mean level. A synth starts every note at another phase, so two renders
/// of one sound are not the same samples, and this is what a filter changes.
...
/// The filter gets the power icon of every effect: it bypasses the slot, the record of the
/// filter stays, and the track sounds as it does without it. One undo step each way.
#[gpui::test]
fn the_power_icon_bypasses_the_filter_as_one_undo_step(cx: &mut TestAppContext) {
    let mut opened = support::open_with(cx, |project| {
        let mut changes = sound_core::Changes::new();
    ...
    let row = opened.control("menu-filter");
    opened.click(row);
    // Low enough to hear: the default synth is bright.
    let cutoff = opened.control("knob-cutoff_hz");
    opened.drag(cutoff, cutoff + point(px(0.), px(150.)));
    let filtered = brightness(&mut opened);
    assert!(filtered < plain * 0.7, "{plain} then {filtered}");

    let power = opened.control("card-filter-power");
    opened.click(power);
    assert_eq!(opened.undo_label().as_deref(), Some("Turn off Filter"));
    let track = std::fs::read_to_string(opened.path("state/arrangement/track-1/instance.json"));
    assert!(
        track
            .unwrap()
            .contains(r#"{"name": "filter", "bypass": true}"#)
    );
    assert!(opened.path(FILTER_FILE).exists());
