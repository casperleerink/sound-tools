...
- Meter: the bars fall 20 dB a second and the peak line waits 1.5 s, in `meter::Ballistics`, which the owner feeds with one peak per frame. The empty bar is `alpha/6`. Above 0 dB the bar stays yellow; the clip light is the red.
