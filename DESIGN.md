...
- The Compressor, step 6 of the third milestone: the display is 136 pt, and the curve takes its left 118 pt, so it is square and a ratio of 1 is the diagonal; input and output are both -60 to 0 dBFS. The gain reduction bar is 5 pt, 6 pt in from the right edge and from top and bottom, right of the curve's end, so the hollow ratio handle at the end of the curve does not cover it. A line at the threshold. The level dot is 8 pt, green, and absent while nothing sounds. The line under the display is `GR -6.8 dB`, to a tenth. The ratio handle stops moving when the threshold is within 1 dB of 0 dBFS, where the line above it has no length; the Ratio knob still works. The hidden columns are Knee over Makeup, then Mix over Lookahead, whose segments are `0 · 1 · 10` with `ms` on the value line.
